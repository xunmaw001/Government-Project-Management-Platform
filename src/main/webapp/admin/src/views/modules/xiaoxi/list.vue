        ...
        <div v-if="showFlag">
            <el-form :inline="true" :model="searchForm" class="form-content">
                <el-row :gutter="20" class="slt" :style="{justifyContent:contents.searchBoxPosition=='1'?'flex-start':contents.searchBoxPosition=='2'?'center':'flex-end'}">
                     ...
                     <el-form-item :label="contents.inputTitle == 1 ? '消息标题' : ''">
                         <el-input prefix-icon="el-icon-search" v-model="searchForm.xiaoxiName" placeholder="消息标题" clearable></el-input>
                     </el-form-item>
                ...
                <el-row class="ad" :style="{justifyContent:contents.btnAdAllBoxPosition=='1'?'flex-start':contents.btnAdAllBoxPosition=='2'?'center':'flex-end'}">
                    <el-form-item>
                        ...
                        <a style="text-decoration:none" class="el-button el-button--success"
                           v-if="isAuth('xiaoxi','导入导出')"
                           icon="el-icon-download"
                           href="http://localhost:8080/xiangmuguanlipingtai/upload/xiaoxiMuBan.xls"
                        >批量导入消息数据模板</a>
                        &nbsp;
                        <el-upload
                                v-if="isAuth('xiaoxi','导入导出')"
                                ...
                                :on-success="xiaoxiUploadSuccess"
                                :on-error="xiaoxiUploadError"
                        ...
                        &nbsp;
                        <!-- 导出excel -->
                        <download-excel v-if="isAuth('xiaoxi','导入导出')" style="display: inline-block" class = "export-excel-wrapper" :data = "dataList" :fields = "json_fields" name = "xiaoxi.xls">
                            <!-- 导出excel -->
                            <el-button
                                    type="success"
                                    icon="el-icon-download"
            ...
            <div class="table-content">
                <el-table class="tables" :size="contents.tableSize" :show-header="contents.tableShowHeader"
                    ...
                    <el-table-column width="300" :align="contents.tableAlign"
                                     ...
                                     label="操作">
                        <template slot-scope="scope">
                            <el-button v-if="isAuth('xiaoxi','查看')" type="success" icon="el-icon-tickets" size="mini" @click="addOrUpdateHandler(scope.row.id,'info')">详情</el-button>
                            <el-button v-if="isAuth('xiaoxi','修改')" type="primary" icon="el-icon-edit" size="mini" @click="addOrUpdateHandler(scope.row.id)">修改</el-button>
                            <el-button v-if="isAuth('xiaoxi','删除')" type="danger" icon="el-icon-delete" size="mini" @click="deleteHandler(scope.row.id)">删除</el-button>
        ...
        methods: {
            chartDialog() {
                ...
                let params = {
                    ...
                    thisTable : {//当前表
                        tableName :"xiaoxi",//当前表表名,
                        sumColum : 'xiaoxi_number', //求和字段
                        date : 'insert_time',//分组日期字段
                        // string : 'xiaoxi_name',//分组字符串字段
                        // types : 'xiaoxi_types',//分组下拉框字段
                    },
                    // joinTable : {//级联表（可以不存在）
            ...
            getDataList() {
                ...
                if (this.searchForm.xiaoxiName!= '' && this.searchForm.xiaoxiName!= undefined) {
                    params['xiaoxiName'] = '%' + this.searchForm.xiaoxiName + '%'
                }
                        
                params['xiaoxiDelete'] = 1// 逻辑删除字段 1 未删除 2 删除
            ...
            // 导入功能上传文件成功后调用导入方法
            xiaoxiUploadSuccess(data){
                let _this = this;
                _this.$http({
                    url: "xiaoxi/batchInsert?fileName=" + data.file,
                    method: "get"
                }).then(({data}) => {
                    if(data && data.code === 0){
                        _this.$message({
